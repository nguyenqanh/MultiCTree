...
class AttU_Net(nn.Module):
    ...
    def _get_padding(self, source, target):
    ...
    def forward(self, x):
        ...
        if d5.shape[2:] != x4.shape[2:]:
            d5 = F.pad(d5, self._get_padding(d5, x4))
        ...
        if d4.shape[2:] != x3.shape[2:]:
            d4 = F.pad(d4, self._get_padding(d4, x3))
        ...
        if d3.shape[2:] != x2.shape[2:]:
            d3 = F.pad(d3, self._get_padding(d3, x2))
        ...
        if d2.shape[2:] != x1.shape[2:]:
            d2 = F.pad(d2, self._get_padding(d2, x1))
