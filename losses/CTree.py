...
def loss_ranked_selection(saliency_measure, importance_measure, num_positives, margin, p=1):
    ...
    if len(saliency_measure) <= num_positives:
        ...
        return 0.2*tc.sum(tc.relu(margin - saliency_measure[:num_positives])**p) + 0.8*tc.sum(saliency_measure[num_positives:]**p)
...
def loss_maxima(graph, image, saliency_measure, importance_measure, num_target_maxima, margin=1, p=1):
  ...
  :return: a torch scalar
  """
  if not saliency_measure in ["altitude", "dynamics", "connect"]:
    raise ValueError("Saliency_measure can be either 'altitude' or 'dynamics' or 'connect'")
  ...
  # connect
  elif saliency_measure == "connect":
      # connect
      depth = attribute_depth(tree, altitudes_np)
      saddle_nodes = tc.from_numpy(attribute_saddle_nodes(tree, depth)[0])
      extrema_connect = 1 - altitudes[saddle_nodes[extrema_indices].long()]
      saliency = extrema_connect
